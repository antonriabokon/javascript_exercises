...
function myAlphabetLength() {
  console.log(myAlphabet.length);  // Logs the length of the array
...
const commonAnimals = reserve1Animals.filter(animal => reserve2Animals.includes(animal));
commonAnimals.forEach(animal => console.log(`Common species: ${animal}`));
...
let isTigerInZoo = zooAnimals.includes('Tiger');
console.log(isTigerInZoo);
...
attendees.forEach((attendee, index) => {
  if (attendee === 'John') johnIndexes.push(index);
});
